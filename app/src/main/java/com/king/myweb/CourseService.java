...
/**
 * Created by 16230 on 2016/9/12.
 */

public class CourseService {
    private HashMap reMap;

    public HashMap getReMap() {
        return reMap;
    }

    public CourseService() {
        reMap = new HashMap<String, Course>();
    }
        ...
        for (int i = 0; i < rowNum - 1; i++) {
            ...
            for (int j = 1; j < columnNum; j++) {
                ...
                if (column.hasAttr("rowspan")) {
                    try {
//                        System.out.println("周" + week + "第" + (i + 1) + "节-第" + (i + Integer.parseInt(column.attr
//                                ("rowspan"))) + "节");
                        String st = "周" + week + " 第" + (i + 1) + "节-第" + (i + Integer.parseInt(column.attr
                                ("rowspan"))) + "节";

                        splitCourse(st, column.html(), startYear, endYear, semester, week, i + 1, i + Integer
                                .parseInt(column.attr("rowspan")));
     ...
     * @return
     */
    private int splitCourse(String st, String str, int startYear, int endYear, int semester, int week, int
            startSection, int endSection) {
        Course course;
        ...
        if (split.length > 1) {// 如果大于一节课
            for (int i = 0; i < split.length; i++) {
                if (!(split[i].startsWith("<br>") && split[i].endsWith("<br>"))) {
                    course = storeCourseByResult(split[i], startYear, endYear, semester, week, startSection,
                            endSection);
                    reMap.put(st, course);
                } else {
                    ...
                    course = storeCourseByResult(substring, startYear, endYear, semester, week, startSection,
                            endSection);
                    reMap.put(st, course);
                    // 保存单节课
                }
            ...
            course = storeCourseByResult(str, startYear, endYear, semester, week, startSection, endSection);// 保存
            reMap.put(st, course);
            return 1;
        }
    }
    ...
    public int getDayOfWeek(String day) {
        if (day.equals("一")) return 1;
        else if (day.equals("二")) return 2;
        else if (day.equals("三")) return 3;
        else if (day.equals("四")) return 4;
        else if (day.equals("五")) return 5;
        else if (day.equals("六")) return 6;
        else if (day.equals("日")) return 7;
        else return 0;
    }

    private Course storeCourseByResult(String sub, int startYear, int endYear, int semester, int week, int
            startSection, int endSection) {
    ...
    public void setEveryWeekByChinese(String week, Course course) {
        // 1代表单周，2代表双周
        if (week != null) {
            if (week.equals("单周")) course.setEveryWeek(1);
            else if (week.equals("双周")) course.setEveryWeek(2);
        }
        // 默认值为0，代表每周
    }
}
